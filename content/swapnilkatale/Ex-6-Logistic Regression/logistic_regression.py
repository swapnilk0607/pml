...
def fit_to_logistic_regression(algorithm, x_train, x_test, y_train, y_test, rows, subplot_count, data):
    ...
    print(
        f" -->> TRAIN -->> Accuracy: {accuracy_train:.4f} |Precision: {precision_train:.4f} |Recall: {recall_train:.4f} |F1_score: {f1score_train:.4f} |ROC: {roc_train:.4f}")
    # Evaluate the model
    ...
    print(
        f" -->> TEST -->> Accuracy: {accuracy_test:.4f} |Precision: {precision_test:.4f} |Recall: {recall_test:.4f} |F1_score: {f1score_test:.4f} |ROC: {roc_test:.4f}")
...
def generate_logistic_regression_plot(algorithm_name, algorithm, x_train, x_test, y_train, y_test, rows, subplot_count,
                                      data):
...
if __name__ == "__main__":
    with pd.ExcelWriter('Logistic_Reg_Output.xlsx',engine='openpyxl') as writer:
        for file in files:
            data = {
                'algorithm_name': [],
                'train_or_test_data': [],
            ...
            for algorithm_name, algorithm in algorithms.items():
                ...
                # plt.tight_layout()
                subplot_count = generate_logistic_regression_plot(algorithm_name, algorithm, x_train, x_test, y_train,
                                                                  y_test, rows, subplot_count, data)
